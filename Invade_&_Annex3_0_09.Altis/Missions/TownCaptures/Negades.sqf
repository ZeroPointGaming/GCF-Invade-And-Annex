...
while {true} do 
	...
	PatrolArea = "CheckpointNagadesMkr";
	
	//Enable the area marker on the map
	PatrolArea setMarkerAlpha 1;
	
	//Create Map marker
	_marker1 = createMarker ["TownAOMarker", getMarkerPos "CheckpointNagadesMkr"];
	...
	//Create Task
	[west,["TownAtkTask"],["CSAT Forces have captured a town! Go and take it back quickly!","Nagades is under attack by CSAT","CheckpointNagadesMkr"],(getMarkerPos "CheckpointNagadesMkr"),0,0,true,"destroy",true] call BIS_fnc_taskCreate;
	...
	_grpa = [getMarkerPos "CheckpointNagadesMkr", EAST, (configFile >> "CfgGroups" >> "East" >> "OPF_F" >> "Infantry" >> "OIA_InfSquad")] call BIS_fnc_spawnGroup;
	_grpaa = [getMarkerPos "CheckpointNagadesMkr", EAST, (configFile >> "CfgGroups" >> "East" >> "OPF_F" >> "Infantry" >> "OIA_InfSquad")] call BIS_fnc_spawnGroup;
	[_grpa, getMarkerPos "CheckpointNagadesMkr", 10] call BIS_fnc_taskDefend;
	[_grpaa, getMarkerPos "CheckpointNagadesMkr", 10] call BIS_fnc_taskDefend;
	...
	if ((random 1) > 0.50) then
		{
			...
			for "_x" from 1 to 1 do {
				...
				_grpar = [getMarkerPos "CheckpointNagadesMkr", EAST, (configFile >> "CfgGroups" >> "East" >> "OPF_F" >> "Infantry" >> "OIA_InfSquad")] call BIS_fnc_spawnGroup;
				[_grpar, getMarkerPos "CheckpointNagadesMkr", 10] call BIS_fnc_taskDefend;
				{_x allowfleeing 0} foreach units _grpar;

				_spawnRandomisationRodopoli=150;
				_spwnposNewRodopoli = [(getMarkerPos "CheckpointNagadesMkr"),random _spawnRandomisationRodopoli,random 360] call BIS_fnc_relPos;
				//_grpar setpos _spwnposNewRodopoli;
				{_x setpos _spwnposNewRodopoli} forEach units _grpar;

				_Posa = getMarkerPos "CheckpointNagadesMkr";
