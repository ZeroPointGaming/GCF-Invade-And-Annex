...
while {true} do 
	...
	PatrolArea = "CheckpointMainAirfieldMkr";
	
	//Enable the area marker on the map
	PatrolArea setMarkerAlpha 1;
	
	//Create Map marker
	_marker1 = createMarker ["TownAOMarker", getMarkerPos "CheckpointMainAirfieldMkr"];
	...
	//Create Task
	[west,["TownAtkTask"],["CSAT Forces have captured a town! Go and take it back quickly!","Main Airfield is under attack by CSAT","CheckpointMainAirfieldMkr"],(getMarkerPos "CheckpointMainAirfieldMkr"),0,0,true,"destroy",true] call BIS_fnc_taskCreate;
	...
	_grpa = [getMarkerPos "CheckpointMainAirfieldMkr", EAST, (configFile >> "CfgGroups" >> "East" >> "OPF_F" >> "Infantry" >> "OIA_InfSquad")] call BIS_fnc_spawnGroup;
	_grpaa = [getMarkerPos "CheckpointMainAirfieldMkr", EAST, (configFile >> "CfgGroups" >> "East" >> "OPF_F" >> "Infantry" >> "OIA_InfSquad")] call BIS_fnc_spawnGroup;
	[_grpa, getMarkerPos "CheckpointMainAirfieldMkr", 10] call BIS_fnc_taskDefend;
	[_grpaa, getMarkerPos "CheckpointMainAirfieldMkr", 10] call BIS_fnc_taskDefend;
	...
	if ((random 1) > 0.50) then
		{
			...
			for "_x" from 1 to 1 do {
				...
				_grpar = [getMarkerPos "CheckpointMainAirfieldMkr", EAST, (configFile >> "CfgGroups" >> "East" >> "OPF_F" >> "Infantry" >> "OIA_InfSquad")] call BIS_fnc_spawnGroup;
				[_grpar, getMarkerPos "CheckpointMainAirfieldMkr", 10] call BIS_fnc_taskDefend;
				{_x allowfleeing 0} foreach units _grpar;

				_spawnRandomisationRodopoli=150;
				_spwnposNewRodopoli = [(getMarkerPos "CheckpointMainAirfieldMkr"),random _spawnRandomisationRodopoli,random 360] call BIS_fnc_relPos;
				//_grpar setpos _spwnposNewRodopoli;
				{_x setpos _spwnposNewRodopoli} forEach units _grpar;

				_Posa = getMarkerPos "CheckpointMainAirfieldMkr";
